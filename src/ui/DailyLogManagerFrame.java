...
public class DailyLogManagerFrame extends JFrame {
    ...
    public DailyLogManagerFrame(DashboardFrame dashboard, int studentId) {
        ...
        add(UITheme.createHeader("Daily Log Manager", "Track what happened today and keep daily totals within 24 hours."), BorderLayout.NORTH);
        ...
        addBtn.addActionListener(e -> showEntryDialog("Add Daily Log", -1, null, null, ""));
    ...
    private void loadData() {
        try {
            ...
            String query =
                    "SELECT MIN(d.log_id) AS log_id, a.activity_name, NVL(sa.subactivity_name, 'General') AS subactivity_name, " +
                    "SUM(d.hours_spent) AS hours_spent, TRUNC(d.log_date) AS log_date " +
                    "FROM daily_log d " +
                    "JOIN activity a ON d.activity_id = a.activity_id " +
                    "LEFT JOIN sub_activity sa ON d.subactivity_id = sa.subactivity_id " +
                    "WHERE d.student_id=? AND TRUNC(d.log_date)=TRUNC(?) " +
                    "GROUP BY a.activity_name, NVL(sa.subactivity_name, 'General'), TRUNC(d.log_date) " +
            ...
            UITheme.showError(this, "Load Failed", "Unable to load daily log data from the database.");
        }
    }
    ...
    private void renderCards() {
        ...
        dateLabel.setText(isToday(selectedDate) ? "Today's Daily Log" : formatter.format(selectedDate));
        totalLabel.setText(formatHours(totalHours) + " logged out of 24 hours");

        if (model.getRowCount() == 0) {
            JPanel emptyCard = createDisplayCard("No daily log yet", "Add what happened on this day to keep your record useful.", "0h", false);
    ...
    private String progressText(double hours) {
        ...
        return percent + "% of the day recorded for this activity";
    }
    ...
    private void showEntryDialog(String title, int recordId, String selectedActivity, String selectedSubActivity, String selectedHours) {
        ...
        form.add(Box.createVerticalStrut(10));
        form.add(createFormRow("Hours Spent:", hoursField));
        ...
        submitBtn.addActionListener(ev -> {
            try {
                ...
                if (hours <= 0 || hours > 24) {
                    UITheme.showInfo(dialog, "Invalid Hours", "Hours must be greater than 0 and less than or equal to 24.");
                    return;
                }
                ...
                UITheme.showError(dialog, "Invalid Number", "Enter a valid number of hours, for example 1.5 or 2.");
            }
        });
    ...
    private boolean isExistingActivityOption(JComboBox<String> activityBox, String value) {
        for (int i = 0; i < activityBox.getItemCount(); i++) {
            ...
            if (!"Other".equals(item) && item.equals(value)) {
                return true;
            }
        }
    ...
    private void addOrMerge(String activity, String subActivity, String customCategory, double hours, JDialog dialog) {
        try {
            ...
            double loggedToday = getTotalHoursForDate(con, selectedDate, -1);
            if (loggedToday + hours > 24) {
                UITheme.showInfo(dialog, "Daily Limit Reached", "This day already has " + loggedToday + " logged hours. Total logged hours cannot exceed 24.");
                return;
            }

            String existingQuery =
                    "SELECT MIN(log_id) AS log_id, NVL(SUM(hours_spent), 0) AS total_hours " +
                    "FROM daily_log WHERE student_id=? AND activity_id=? AND subactivity_id=? AND TRUNC(log_date)=TRUNC(?)";
            ...
            if (existingRs.next()) {
                existingId = existingRs.getInt("log_id");
                existingHours = existingRs.getDouble("total_hours");
            }

            if (existingId > 0) {
                String updateQuery = "UPDATE daily_log SET hours_spent=? WHERE log_id=? AND student_id=?";
                ...
                String insertQuery = "INSERT INTO daily_log (log_id, student_id, activity_id, log_date, hours_spent, subactivity_id) VALUES (log_seq.NEXTVAL, ?, ?, ?, ?, ?)";
                ...
                UITheme.showSuccess(dialog, "Log Added", "The daily log item was added successfully.");
            }
            ...
            System.out.println(ex);
            UITheme.showError(dialog, "Save Failed", "Unable to save the daily log item.");
        }
    }
    ...
    private void updateSelected() {
        int row = selectedRow;
        if (row == -1) {
            UITheme.showInfo(this, "No Log Selected", "Select a daily log card before clicking Update Selected.");
            return;
        }
        ...
        showEntryDialog("Update Daily Log", id, activity, subActivity, hours);
    }
    ...
    private void updateRecord(int recordId, double hours, JDialog dialog) {
        try {
            ...
            String infoQuery = "SELECT activity_id, subactivity_id, TRUNC(log_date) AS log_date FROM daily_log WHERE log_id=? AND student_id=?";
            ...
            if (!infoRs.next()) {
                UITheme.showError(dialog, "Not Found", "Selected log was not found.");
                return;
            }

            int activityId = infoRs.getInt("activity_id");
            Date logDate = infoRs.getDate("log_date");
            double otherHours = getTotalHoursForDate(con, logDate, recordId);

            if (otherHours + hours > 24) {
                UITheme.showInfo(dialog, "Daily Limit Reached", "Other logged items already use " + otherHours + " hours. Total logged hours cannot exceed 24.");
                return;
            }
            ...
            Integer subActivityId = getLogSubActivityId(con, recordId);
            removeDuplicateRows(con, recordId, activityId, subActivityId, logDate);

            UITheme.showSuccess(dialog, "Log Updated", "The selected daily log time was updated successfully.");
            dialog.dispose();
            loadData();

        } catch (Exception ex) {
            System.out.println(ex);
            UITheme.showError(dialog, "Update Failed", "Unable to update the selected daily log item.");
        }
    }
    ...
    private void deleteSelected() {
        ...
        if (row == -1) {
            UITheme.showInfo(this, "No Log Selected", "Select a daily log card before deleting.");
            return;
        }
        ...
        try {
            ...
            String infoQuery = "SELECT activity_id, TRUNC(log_date) AS log_date FROM daily_log WHERE log_id=? AND student_id=?";
            PreparedStatement infoPst = con.prepareStatement(infoQuery);
            infoPst.setInt(1, id);
            infoPst.setInt(2, studentId);
            ResultSet infoRs = infoPst.executeQuery();
            ...
            if (!infoRs.next()) {
                UITheme.showError(this, "Not Found", "Selected log was not found.");
                return;
            }

            String q = "DELETE FROM daily_log WHERE student_id=? AND activity_id=? AND (subactivity_id=? OR (subactivity_id IS NULL AND ? IS NULL)) AND TRUNC(log_date)=TRUNC(?)";
            ...
            pst.setDate(5, infoRs.getDate("log_date"));
            pst.executeUpdate();

            UITheme.showSuccess(this, "Log Deleted", "The selected daily log entry was deleted.");
            loadData();

        } catch (Exception ex) {
            System.out.println(ex);
            UITheme.showError(this, "Delete Failed", "Unable to delete the selected daily log item.");
        }
    }
    ...
    private Integer getLogSubActivityId(Connection con, int recordId) throws SQLException {
        String query = "SELECT subactivity_id FROM daily_log WHERE log_id=? AND student_id=?";
        PreparedStatement pst = con.prepareStatement(query);
    ...
    private double getTotalHoursForDate(Connection con, Date date, int excludeId) throws SQLException {
        ...
        if (date == null) {
            String query =
                    "SELECT NVL(SUM(hours_spent), 0) FROM daily_log " +
                    "WHERE student_id=? AND TRUNC(log_date)=TRUNC(SYSDATE) AND log_id<>?";
            ...
            String query =
                    "SELECT NVL(SUM(hours_spent), 0) FROM daily_log " +
                    "WHERE student_id=? AND TRUNC(log_date)=TRUNC(?) AND log_id<>?";
            pst = con.prepareStatement(query);
            pst.setInt(1, studentId);
            pst.setDate(2, date);
    ...
    private double getTotalHoursForDateExcludingActivity(Connection con, Date date, int activityId) throws SQLException {
        String query =
                "SELECT NVL(SUM(hours_spent), 0) FROM daily_log " +
                "WHERE student_id=? AND TRUNC(log_date)=TRUNC(?) AND activity_id<>?";
        PreparedStatement pst = con.prepareStatement(query);
        pst.setInt(1, studentId);
        pst.setDate(2, date);
    ...
    private void removeDuplicateRows(Connection con, int keepId, int activityId, Integer subActivityId, Date date) throws SQLException {
        ...
        if (date == null) {
            String deleteQuery =
                    ...
                    "WHERE student_id=? AND activity_id=? AND (subactivity_id=? OR (subactivity_id IS NULL AND ? IS NULL)) AND TRUNC(log_date)=TRUNC(SYSDATE) AND log_id<>?";
            ...
            String deleteQuery =
                    "DELETE FROM daily_log " +
                    "WHERE student_id=? AND activity_id=? AND (subactivity_id=? OR (subactivity_id IS NULL AND ? IS NULL)) AND TRUNC(log_date)=TRUNC(?) AND log_id<>?";
            deletePst = con.prepareStatement(deleteQuery);
            deletePst.setInt(1, studentId);
            deletePst.setInt(2, activityId);
            setNullableInt(deletePst, 3, subActivityId);
